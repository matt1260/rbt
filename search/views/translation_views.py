...
def start_translation_job(request):
    ...
    try:
        ...
        # Additional safeguard: check whether any verses/footnotes actually need translation
        ...
        if language != 'en':
            ...
            if book in new_testament_books:
                ...
                chapter_rows = results.get('chapter_reader') or []
                for bk, ch_num, vrs, html_verse in chapter_rows:
                    if int(vrs) not in existing_translations:
                ...
                if not needs_translation:
                    # Collect footnote ids from HTML
                    # Resolve book abbreviation(s) for footnote id formats
                try:
                    from translate.translator import book_abbreviations, convert_book_name
                    book_abbrev = book_abbreviations.get(book) or book_abbreviations.get(convert_book_name(book) or '')
                except Exception:
                    book_abbrev = None

                for bk, ch_num, vrs, html_verse in chapter_rows:
                    sup_texts = re.findall(r'<sup>(.*?)</sup>', html_verse or '')
                    for sup in sup_texts:
                        candidate_ids = [f"{book}-{sup}"]
                        if book_abbrev:
                            candidate_ids.insert(0, f"{book_abbrev}-{sup}")
                        # If any candidate id is present as a completed translation, it's OK
                        found = False
                        for cid in candidate_ids:
                            if VerseTranslation.objects.filter(book=book, chapter=chapter_num, language_code=language, footnote_id=cid, status='completed').exists():
                                found = True
                                break
                        if not found:
                            needs_translation = True
            ...
            # For OT and Genesis, reuse existing logic in chapter handlers
            else:
                # Genesis / OT use different data structures; call the chapter handler logic
                # Simplest approach: compare counts
                existing_translations_count = VerseTranslation.objects.filter(book=book, chapter=chapter_num, language_code=language, status='completed', footnote_id__isnull=True).count()
                total_verses = 0
                if book == 'Genesis' and results.get('rbt'):
                    total_verses = len(results.get('rbt'))
                elif results.get('html'):
                    total_verses = sum(1 for v in (results.get('html') or {}).values())
                if existing_translations_count < total_verses:
                    needs_translation = True

        # If translation is not needed, return early to prevent creating jobs
        if not needs_translation:
            logger.info(f"Skipping translation job for {book} ch{chapter_num} lang={language}: already translated or cached")
            return JsonResponse({'status': 'ok', 'job_id': None, 'message': 'No translation required; cached.'})
